...
import {
  ...
  WRITABLE_CONDITION_LEDGER_SCHEMA,
...
import { CANONICAL_BODY_ROUTES, CANONICAL_RUNTIME_STAGE_ORDER, resolveCanonicalRuntimeRoute } from "../src/canonical-runtime-route.js";
import { CANONICAL_BODY_SEQUENCES, CANONICAL_SKILL_CHAINS, RUNTIME_SKILL_IDS, bodySequenceForRoute, skillChainForBodyRoute } from "../src/runtime-skill-chain.js";
import { CURRENT_MODULE_MAP, RUNTIME_ARCHITECTURE_ID, describeRuntimeArchitecture } from "../src/runtime-architecture-registry.js";
import { buildRuntimeExecutionPlan, validateRuntimeExecutionTrace } from "../src/runtime-execution-plan.js";
import { ARTIFACT_POLICY, REPORT_POLICY, RUNTIME_STATE_IDS, SUCCESS_AUTHORITY } from "../src/runtime-vocabulary.js";
import { auditRuntimeTerminology } from "../src/runtime-terminology-audit.js";

test("runtime version and policy are repair locked", () => {
  assert.equal(RUNTIME_VERSION, "pw90-v004.28-nlcore-story-layer-v28-bound");
  ...
  assert.equal(RUNTIME_POLICY.lightweightWriteSupportedCurrentRuntime, false);
  assert.equal(RUNTIME_POLICY.explicitLightweightRequestStopsBeforeText, true);
  ...
  assert.equal(RUNTIME_POLICY.secondDraftTargetBodyChars, 15000);
  assert.equal(RUNTIME_POLICY.secondDraft15kHardFloor, false);
  assert.equal(RUNTIME_POLICY.secondDraftUnder15kRequiresFullBurnProof, true);
  assert.equal(RUNTIME_POLICY.secondDraftLengthPaddingDenied, true);
  ...
  assert.equal(RUNTIME_POLICY.writerLayerDefault, "ON");
  assert.equal(RUNTIME_POLICY.writerLayerUnspecifiedDefaultsOn, true);
  assert.equal(RUNTIME_POLICY.writerLayerOffRequiresExplicitDeclaration, true);
  assert.equal(RUNTIME_POLICY.novelFirstSeedBurnPhilosophyEnabled, true);
  assert.equal(RUNTIME_POLICY.explicitHoldExcludedFromBurnObligation, true);
  assert.equal(RUNTIME_POLICY.usableSeedFullBurnRequired, true);
  assert.equal(RUNTIME_POLICY.aiDetectorScoreAsSuccessCriterionDenied, true);
  assert.equal(RUNTIME_POLICY.runtimeArchitectureObservationalOnly, true);
  assert.equal(RUNTIME_POLICY.packOnlyWriteBehaviorBaselinePreserved, true);
  assert.equal(RUNTIME_POLICY.mandatoryInternalRedraftCount, 0);
  assert.equal(RUNTIME_POLICY.forcedDraftAUnburnedRequirement, false);
  assert.equal(RUNTIME_POLICY.fixedFinalUserReportAdapterRequired, false);
});

test("all source files are present in original read order", () => {
  assert.deepEqual(READ_ORDER, SOURCE_MANIFEST.map((entry) => entry.path));
  assert.equal(SOURCE_MANIFEST.length, 23);
});

for (const expected of SOURCE_MANIFEST) {
  test(`${expected.path} matches the current source manifest`, () => {
...
test("pack plus body text activates second draft full-burn expansion with 15K target and exact body-head directive", () => {
  const lock = readFileSync(new URL("../source/knowledge/PW90_SECOND_DRAFT_FULL_BURN_EXPANSION_LOCK_v001.md", import.meta.url), "utf8");
  assert.equal(SECOND_DRAFT_BRANCH_LOCK_ID, "PW90_SECOND_DRAFT_FULL_BURN_EXPANSION_LOCK");
  assert.equal(SECOND_DRAFT_TARGET_BODY_CHARS, 15000);
  assert.ok(lock.includes("話パックのみ = 21b通常WRITEをそのまま使う"));
  assert.ok(lock.includes("話パック + 本文TXT = 二稿増補分岐"));
  assert.ok(lock.includes("【二稿増補指示】"));
  assert.ok(READ_ORDER.includes("source/knowledge/PW90_SECOND_DRAFT_FULL_BURN_EXPANSION_LOCK_v001.md"));
  ...
  assert.equal(gate.targetBodyChars, 15000);
  assert.equal(gate.hardMinBodyChars, null);
  ...
  assert.match(gate.targetLengthOrSelfBound, /15K級を目安/);
  assert.match(gate.targetLengthOrSelfBound, /水増し禁止/);
  assert.deepEqual(validateSecondDraftBranch({ gate }), []);
...
test("second draft under 15K uses full-burn proof path and 15K does not bypass quality gates", () => {
  const common = {
  ...
  const shortText = `${SECOND_DRAFT_BODY_HEAD_DIRECTIVE}
# 第081話　白い息の苦情
${"あ".repeat(14999)}`;
  const longText = `${SECOND_DRAFT_BODY_HEAD_DIRECTIVE}
# 第081話　白い息の苦情
${"あ".repeat(15000)}`;
  const underNoProof = validateSecondDraftPostText({ gate: { ...common, actualBodyCharCount: 999999 }, text: shortText });
  assert.ok(underNoProof.some((entry) => entry.code === "SECOND_DRAFT_UNDER15K_FULL_BURN_PROOF_REQUIRED"));
  assert.deepEqual(validateSecondDraftPostText({ gate: { ...common, actualBodyCharCount: 999999, under15kFullBurnProof: "all usable pack material burned; further expansion would be padding" }, text: shortText }), []);
  assert.deepEqual(validateSecondDraftPostText({ gate: { ...common, actualBodyCharCount: 1 }, text: longText }), []);
  assert.ok(validateSecondDraftPostText({ gate: { ...common, lengthPaddingDetected: true }, text: longText }).some((entry) => entry.code === "SECOND_DRAFT_LENGTH_PADDING_DENIED"));
});

test("novel-first seed-burn philosophy and story-layer v28 canon are mounted", () => {
  const philosophyPath = "source/knowledge/PW90_NOVEL_FIRST_SEED_BURN_PHILOSOPHY_v002.md";
  const v28Path = "source/knowledge/layer_runtime_v28_ai_native_complete_candidate.md";
  const adapterPath = "source/knowledge/PW90_LAYER_V28_WRITER_ADAPTER_v001.md";
  const v21Path = "source/knowledge/話レイヤー再定義版_マルチ運用_v21_既存例統合_心理内圧_年齢性別_描写没入快感.md";
  const legacyNarration = "source/knowledge/NARRATION_LAYER_MULTI_OPERATION.md";
  const philosophy = readFileSync(new URL(`../${philosophyPath}`, import.meta.url), "utf8");
  const v28 = readFileSync(new URL(`../${v28Path}`, import.meta.url), "utf8");
  const v21 = readFileSync(new URL(`../${v21Path}`, import.meta.url), "utf8");
  assert.ok(READ_ORDER.includes(philosophyPath));
  assert.equal(READ_ORDER.indexOf(v28Path), READ_ORDER.indexOf(philosophyPath) + 1);
  assert.equal(READ_ORDER.indexOf(adapterPath), READ_ORDER.indexOf(v28Path) + 1);
  assert.equal(READ_ORDER.includes(v21Path), false);
  assert.equal(READ_ORDER.includes(legacyNarration), false);
  assert.ok(philosophy.includes("現行実行基準は v28"));
  assert.ok(v28.includes("STATUS: ACTIVE_CANONICAL_WITH_ALL_ITEM_MEANING_AND_PRESET_REFERENCE"));
  assert.ok(v28.includes("embedding_rules_for_character_design"));
  assert.ok(v28.includes("embedding_rules_for_world_axis"));
  assert.ok(v28.includes("LOAD_ORDER_FOR_WRITER"));
  assert.ok(v28.includes("CHARACTER_DESIGN_EMBED_SCHEMA"));
  assert.ok(v28.includes("WORLD_AXIS_EMBED_SCHEMA"));
  assert.ok(v21.includes("作品横断で使える共通定義"));
  assert.equal(RUNTIME_POLICY.storyLayerV28CanonicalEnabled, true);
  assert.equal(RUNTIME_POLICY.storyLayerV28ProfileValueSeparationRequired, true);
  assert.equal(RUNTIME_POLICY.storyLayerV28RuntimeMutationDenied, true);
  assert.equal(RUNTIME_POLICY.storyLayerV28CharacterWorldEmbedSemanticsEnabled, true);
  assert.equal(RUNTIME_POLICY.storyLayerV21LineageReferenceOnly, true);
  assert.equal(RUNTIME_POLICY.legacyLayerTextActiveMergeDenied, true);
  assert.equal(RUNTIME_POLICY.narrationLayerMultiOperationActiveRuntime, false);
});

test("prose density grounding lock prevents dialogue and action settings from silently thinning narration", () => {
  const lockPath = "source/knowledge/PW90_PROSE_DENSITY_GROUNDING_LOCK_v001.md";
  const adapterPath = "source/knowledge/PW90_LAYER_V28_WRITER_ADAPTER_v001.md";
  const lock = readFileSync(new URL(`../${lockPath}`, import.meta.url), "utf8");
  assert.ok(READ_ORDER.includes(lockPath));
  assert.equal(READ_ORDER.indexOf(lockPath), READ_ORDER.indexOf(adapterPath) + 1);
  assert.ok(lock.includes("exchange_heavy"));
  assert.ok(lock.includes("prose: medium"));
  assert.ok(lock.includes("explanation_low -> prose_low"));
  assert.ok(lock.includes("immediate_action -> one_action_one_sentence"));
  assert.ok(lock.includes("OBJECT_FIRST -> sentence_fragmentation"));
  assert.ok(lock.includes("GROUND_PROSE_THINNESS"));
  assert.ok(lock.includes("every_N_dialogue_turns_insert_prose"));
  assert.equal(RUNTIME_POLICY.proseDensityGroundingLockEnabled, true);
  assert.equal(RUNTIME_POLICY.dialogueProseRatioIsForegroundBalanceNotLineRatio, true);
  assert.equal(RUNTIME_POLICY.exchangeHeavyRequiresProseMediumMeaning, true);
  assert.equal(RUNTIME_POLICY.explanationLowDoesNotMeanProseLow, true);
  assert.equal(RUNTIME_POLICY.immediateActionDoesNotRequireMicroSentences, true);
  assert.equal(RUNTIME_POLICY.objectFirstDoesNotRequireSentenceFragmentation, true);
  assert.equal(RUNTIME_POLICY.dialogueVerticalLengthAccepted, true);
  assert.equal(RUNTIME_POLICY.scriptLikeDialoguePingPongDenied, true);
  assert.equal(RUNTIME_POLICY.groundProseThinnessSuccessDenied, true);
  assert.equal(RUNTIME_POLICY.fixedDialogueTurnIntervalInsertionDenied, true);
});
...
test("default write mode is fullburn without emphasis word", () => {
  ...
  assert.equal(light.decision, "STOP_BEFORE_TEXT");
  assert.equal(light.writeMode, null);
  assert.ok(light.failures.some((entry) => entry.code === "LIGHTWEIGHT_MODE_UNSUPPORTED_CURRENT_RUNTIME"));
});
...
test("no-thin knowledge and runtime agree on the only under-15K proof reason", () => {
  const lock = readFileSync(new URL("../source/knowledge/PW90_FULL_POWER_NO_THIN_SUCCESS_LOCK_v001.md", import.meta.url), "utf8");
  assert.ok(lock.includes("under15k_full_burn_proven_no_remaining_material"));
  assert.equal(lock.includes("not_underlength / natural_compression_after_full_recovery"), false);
...
test("operation lock separates supported maintenance from writing and rejects unresolved legacy maintenance", () => {
  assert.equal(evaluateOperationLock({ mode: "BOOT", noManuscriptRequested: true }).decision, "OPERATION_ALLOWED");
  for (const mode of ["INSPECT", "REPAIR"]) {
    const unsupported = evaluateOperationLock({ mode, noManuscriptRequested: true });
    assert.equal(unsupported.decision, "STOP_BEFORE_TEXT");
    assert.ok(unsupported.failures.some((entry) => entry.code === "OPERATION_MODE_UNSUPPORTED_CURRENT_RUNTIME"));
  }
...
test("writer layer policy defaults ON when unspecified and does not block WRITE", () => {
  assert.equal(DEFAULT_WRITER_LAYER_POLICY, "ON");
  const layer = resolveWriterLayerPolicy({});
  assert.equal(layer.decision, "LAYER_POLICY_RESOLVED");
  assert.equal(layer.layerPolicy, "ON");
  assert.equal(layer.source, "DEFAULT_ON_WHEN_UNSPECIFIED");
  const result = evaluateOperationLock({
    mode: "WRITE",
    writerPackOrHandoffPresent: true,
    targetStoryNumberPresent: true,
    outputFormatPresent: true
  });
  assert.equal(result.decision, "OPERATION_ALLOWED");
  assert.equal(result.resolvedLayerPolicy, "ON");
});

test("writer layer OFF requires an explicit declaration", () => {
  const blocked = evaluateOperationLock({
    mode: "WRITE",
    writerPackOrHandoffPresent: true,
    targetStoryNumberPresent: true,
    outputFormatPresent: true,
  ...
  assert.equal(blocked.decision, "STOP_BEFORE_TEXT");
  assert.ok(blocked.failures.some((entry) => entry.code === "LAYER_OFF_REQUIRES_EXPLICIT_DECLARATION"));
  const allowed = evaluateOperationLock({
    mode: "WRITE",
    writerPackOrHandoffPresent: true,
    targetStoryNumberPresent: true,
    outputFormatPresent: true,
    layerPolicy: "OFF",
    layerOffExplicit: true
  });
  assert.equal(allowed.decision, "OPERATION_ALLOWED");
  assert.equal(allowed.resolvedLayerPolicy, "OFF");
});

const canonicalWriteOperation = (extra = {}) => ({
  mode: "WRITE",
  writerPackOrHandoffPresent: true,
  targetStoryNumberPresent: true,
  outputFormatPresent: true,
...
test("architecture route preserves the PACK_ONLY baseline without mandatory redraft", () => {
  const route = resolveCanonicalRuntimeRoute({ operation: canonicalWriteOperation(), writeTrigger: "write", hasWritableStoryPack: true });
  assert.equal(route.decision, "WRITE_ROUTE_RESOLVED");
  assert.equal(route.bodyRoute, CANONICAL_BODY_ROUTES.PACK_ONLY_FULLBURN);
  assert.equal(route.bodyRoute, "PACK_ONLY_FULLBURN_21C_BASELINE");
  assert.deepEqual(route.bodySequence, ["FULLBURN_BODY_WRITE_21C", "SUCCESS_GATE"]);
  assert.equal(route.resolvedLayerPolicy, "ON");
  assert.equal(route.internalConvergenceRequired, undefined);
  assert.equal(skillChainForBodyRoute(route.bodyRoute).includes("S05_INTERNAL_CONVERGENCE_REDRAFT"), false);
});

test("architecture route preserves the existing external second-draft branch", () => {
  const route = resolveCanonicalRuntimeRoute({ operation: canonicalWriteOperation(), writeTrigger: "write", hasWritableStoryPack: true, hasBodyTxt: true });
  assert.equal(route.bodyRoute, CANONICAL_BODY_ROUTES.PACK_PLUS_BODY_TEXT);
  assert.equal(route.secondDraftBranch, "ACTIVE");
  assert.equal(route.externalSecondDraftRequired, true);
  assert.deepEqual(bodySequenceForRoute(route.bodyRoute), ["SECOND_DRAFT_BRANCH", "SUCCESS_GATE"]);
  assert.ok(skillChainForBodyRoute(route.bodyRoute).includes(RUNTIME_SKILL_IDS.EXTERNAL_SECOND_DRAFT));
});

test("architecture modules organize but do not create manuscript or SUCCESS", () => {
  assert.equal(RUNTIME_ARCHITECTURE_ID, "PW90_RUNTIME_ARCHITECTURE_CONTRACT_EXECUTION_HARDENED");
  const route = resolveCanonicalRuntimeRoute({ operation: canonicalWriteOperation(), writeTrigger: "write", hasWritableStoryPack: true });
  const before = JSON.stringify(route);
  const desc = describeRuntimeArchitecture(route);
  assert.equal(desc.createsSuccess, false);
  assert.equal(desc.mutatesRoute, false);
  assert.equal(desc.changesWriteBehavior, false);
  assert.equal(JSON.stringify(route), before);
  const plan = buildRuntimeExecutionPlan({ operation: canonicalWriteOperation(), writeTrigger: "write", hasWritableStoryPack: true });
  assert.equal(plan.planCreatesManuscript, false);
  assert.equal(plan.planCreatesSuccess, false);
  assert.equal(plan.planChangesWriteBehavior, false);
  assert.strictEqual(plan.successOrchestrator, SUCCESS_AUTHORITY);
  assert.strictEqual(plan.artifactPolicy, ARTIFACT_POLICY);
  assert.strictEqual(plan.reportPolicy, REPORT_POLICY);
});

test("BOOT stays body-free while unresolved INSPECT and REPAIR are rejected", () => {
  const boot = resolveCanonicalRuntimeRoute({ operation: { mode: "BOOT", noManuscriptRequested: true } });
  assert.equal(boot.bodyRoute, CANONICAL_BODY_ROUTES.MAINTENANCE);
  assert.equal(boot.manuscriptTextAllowed, false);
  assert.deepEqual(boot.canonicalStages, ["START_HERE", "BOOT", "WRITE_ROUTE_RESOLUTION"]);
  for (const mode of ["INSPECT", "REPAIR"]) {
    const route = resolveCanonicalRuntimeRoute({ operation: { mode, noManuscriptRequested: true } });
    assert.equal(route.decision, "STOP_BEFORE_TEXT");
    assert.equal(route.bodyRoute, null);
    assert.ok(route.failures.some((entry) => entry.code === "OPERATION_MODE_UNSUPPORTED_CURRENT_RUNTIME"));
  }
  assert.deepEqual(CANONICAL_SKILL_CHAINS[CANONICAL_BODY_ROUTES.MAINTENANCE], [RUNTIME_SKILL_IDS.MOUNT_BOOT]);
});

test("execution plan trace detects route and terminal drift", () => {
  const plan = buildRuntimeExecutionPlan({ operation: canonicalWriteOperation(), writeTrigger: "write", hasWritableStoryPack: true });
  const valid = { bodyRoute: plan.bodyRoute, canonicalStages: plan.canonicalStages, skillChain: plan.skillChain, requiredGuards: plan.requiredGuards, maintenanceOperation: plan.maintenanceOperation, successAuthority: plan.successOrchestrator, terminalState: RUNTIME_STATE_IDS.SUCCESS, artifactPreparedBeforeFullConvergence: false, planCreatesManuscript: false, planCreatesSuccess: false };
  assert.equal(validateRuntimeExecutionTrace(plan, valid).decision, "EXECUTION_TRACE_ACCEPTED");
  const bad = validateRuntimeExecutionTrace(plan, { ...valid, bodyRoute: CANONICAL_BODY_ROUTES.PACK_PLUS_BODY_TEXT });
  assert.equal(bad.decision, "EXECUTION_TRACE_REJECTED");
  assert.ok(bad.failures.some((entry) => entry.code === "EXECUTION_BODY_ROUTE_MISMATCH"));
});

test("architecture classifies every src module and terminology audit converges", () => {
  const classified = new Set(Object.values(CURRENT_MODULE_MAP).flat().filter((entry) => entry.startsWith("src/")).map((entry) => entry.split("#")[0]));
  const actual = Array.from(new URL("../src/", import.meta.url).pathname ? [] : []);
  const source = readFileSync(new URL("../src/runtime-architecture-registry.js", import.meta.url), "utf8");
  assert.ok(source.includes("CURRENT_MODULE_MAP"));
  for (const rel of ["src/canonical-runtime-route.js", "src/runtime-route-contract.js", "src/runtime-skill-chain.js", "src/runtime-execution-plan.js", "src/runtime-vocabulary.js", "src/runtime-terminology-audit.js"]) assert.ok(classified.has(rel));
...
test("architecture stage order is stable and does not encode Draft-A/Draft-B", () => {
  assert.deepEqual(CANONICAL_RUNTIME_STAGE_ORDER, ["START_HERE", "BOOT", "WRITE_ROUTE_RESOLUTION", "PREWRITE", "BODY_ROUTE", "POSTTEXT", "ARTIFACT", "USER_DELIVERY"]);
  assert.deepEqual(CANONICAL_BODY_SEQUENCES[CANONICAL_BODY_ROUTES.PACK_ONLY_FULLBURN], ["FULLBURN_BODY_WRITE_21C", "SUCCESS_GATE"]);
  assert.equal(JSON.stringify(CANONICAL_SKILL_CHAINS).includes("INTERNAL_CONVERGENCE_REDRAFT"), false);
});
...
test("writable ZIP story pack minimum accepts a txt-only pack and warns rather than rejecting non-canonical format", () => {
  ...
  try {
    ...
    assert.equal(result.writeDecision, "WRITABLE_STORY_PACK_PREWRITE_EXTRACTION_REQUIRED");
    ...
    assert.equal(result.shelves, null);
    assert.equal(result.bodyEligibleFileCount, 1);
    assert.equal(result.fileCatalog.find((entry) => entry.path === "story.txt").role, "RESTORE_SOURCE_CANDIDATE");
  } finally {
    rmSync(tmp, { recursive: true, force: true });
  }
});
...
test("writable ZIP story pack prewrite requires condition-level extraction, hard binding, pickup and freeze proof", () => {
  const tmp = mkdtempSync(join(tmpdir(), "pw-writable-prewrite-"));
  try {
    const content = "少女は古い灯台で、祖母の手紙を読む。禁止線と接続と話層と出典を全て条件として回収する。\n";
    writeFixtureFile(tmp, "single.txt", content);
    const pack = inspectWritableStoryPackDirectory(tmp);
    const sha = digest(content);
    const conditionIds = conditionIdsForEpisode("PACK");
    const pre = evaluateWritableStoryPackPreWrite({
      activation: {
        input_mode: WRITABLE_STORY_PACK_INPUT_MODE,
        activation_id: "PW-ACT-PACK",
        episode_id: "PACK",
        degraded_mode: false,
        activation_validity: {
          user_or_design_declared: true,
        ...
        designer_runtime_version: "provenance-only"
      },
      hardBindingState: validHardBindingState(),
      writableStoryPackResult: pack,
      writableConditionLedger: {
        schema: WRITABLE_CONDITION_LEDGER_SCHEMA,
        sourceCoverage: [{ path: "single.txt", sha256: sha, extractionComplete: true }],
        conditions: [
          { id: "REQ_PACK_001", role: "RESTORE_SOURCE", sourcePath: "single.txt", sourceLocator: "single.txt#required", conditionText: "祖母の手紙を読む" },
          { id: "FORBID_PACK_001", role: "RESTORE_CONSTRAINT", sourcePath: "single.txt", sourceLocator: "single.txt#forbid", conditionText: "禁止線を踏まない" },
          { id: "LAYER_PACK_001", role: "RESTORE_CONSTRAINT", sourcePath: "single.txt", sourceLocator: "single.txt#layer", conditionText: "話層を適用する" },
          { id: "CONN_PACK_001", role: "RESTORE_CONSTRAINT", sourcePath: "single.txt", sourceLocator: "single.txt#connection", conditionText: "次話接続を保持する" },
          { id: "SRC_PACK_001", role: "RESTORE_CONSTRAINT", sourcePath: "single.txt", sourceLocator: "single.txt#source", conditionText: "出典を確認する" }
        ]
      },
      preTextPickup: validPreTextPickup({
        restoreIds: ["WZ_REQ_PACK_001"],
        constraintIds: ["WZ_FORBID_PACK_001", "WZ_LAYER_PACK_001", "WZ_CONN_PACK_001", "WZ_SRC_PACK_001"],
        episode: "PACK"
      }),
      fullPowerWriteGate: validFullPowerPreWriteGate(conditionIds),
      preWrite: Object.fromEntries([
        ...REQUIRED_PREWRITE_FLAGS.map((field) => [field, true]),
        ["executionOrder", [...WRITE_EXECUTION_ORDER]]
      ])
    ...
    assert.equal(pre.shelves.RESTORE_CONSTRAINT.length, 4);
    assert.deepEqual(pre.pickupConditionIds, conditionIds);
  } finally {
    rmSync(tmp, { recursive: true, force: true });
  }
});
...
test("full power no-thin success lock is mounted and exported", () => {
  ...
  assert.deepEqual(validateFullPowerPostTextGate({ preWriteResult: pre, gate: validFullPowerPostTextGate(), bodyText: "あ".repeat(15000) }), []);
});
...
test("episode 15K full-use beta lock is mounted and enforced", () => {
  ...
  const postFailures = validateFullPowerPostTextGate({ preWriteResult: pre, gate: postGate, bodyText: "あ".repeat(5000) });
...
test("normal FULLBURN observed 14999 can reach SUCCESS when all burn and convergence evidence passes", () => {
  const input = validWriterInput();
  const pre = evaluateV2PreWrite(input);
  assert.equal(pre.decision, "WRITE_ALLOWED");
  const output = validOutput();
  output.text = "あ".repeat(14999);
  const checks = validChecks();
  checks.fullPowerWriteLock.textScaleAudit.actualCharCount = 999999;
  checks.fullPowerWriteLock.textScaleAudit.under15kFullBurnProof = "all pack material burned; no remaining natural expansion";
  const result = evaluateOutputGate({ preWriteResult: pre, activation: input.activation, output, checks, consumption: validConsumption() });
  assert.equal(result.decision, "SUCCESS");
});

test("normal FULLBURN observed 5000 cannot hide behind claimed actualCharCount 15000", () => {
  const input = validWriterInput();
  const pre = evaluateV2PreWrite(input);
  assert.equal(pre.decision, "WRITE_ALLOWED");
  const output = validOutput();
  output.text = "あ".repeat(5000);
  const checks = validChecks();
  checks.fullPowerWriteLock.textScaleAudit.actualCharCount = 15000;
  checks.fullPowerWriteLock.textScaleAudit.under15kFullBurnProof = "";
  const result = evaluateOutputGate({ preWriteResult: pre, activation: input.activation, output, checks, consumption: validConsumption() });
  assert.equal(result.decision, "FAILED_TEXT_QUARANTINE");
  assert.ok(result.failures.some((x) => x.code === "EPISODE_15K_UNDER15K_FULL_BURN_PROOF_REQUIRED" && x.detail?.observedBodyCharCount === 5000));
});
