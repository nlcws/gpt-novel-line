...
export function evaluateNoChatDisplayCompressionStatus({
  ...
  const effectiveObservedCount = Number.isFinite(observedBodyCharCount) ? observedBodyCharCount : null;
  const underTarget = Number.isFinite(effectiveObservedCount) && effectiveObservedCount < targetCharCount;
