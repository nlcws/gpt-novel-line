import { observeNormalFullburnBody } from "./body-char-observer.js";
...
export function validateFullPowerPostTextGate({ preWriteResult, gate, bodyText } = {}) {
  const failures = [];
  if (gate == null || typeof gate !== "object" || Array.isArray(gate)) {
  ...
  if (scaleAudit == null || typeof scaleAudit !== "object" || Array.isArray(scaleAudit)) {
    ...
    const observation = observeNormalFullburnBody(bodyText);
    ...
    if (Number.isFinite(observation.observedBodyCharCount) && observation.observedBodyCharCount < 15000 && !hasText(scaleAudit.under15kFullBurnProof)) {
      failures.push(fail("EPISODE_15K_UNDER15K_FULL_BURN_PROOF_REQUIRED", "checks.fullPowerWriteLock.textScaleAudit.under15kFullBurnProof", { observedBodyCharCount: observation.observedBodyCharCount }));
    }
  }
