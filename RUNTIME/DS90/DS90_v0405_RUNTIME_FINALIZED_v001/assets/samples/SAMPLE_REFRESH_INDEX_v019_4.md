# SAMPLE_REFRESH_INDEX_v019_4

STATUS: ACTIVE_STABLE_ROUTE_SCHEMA
...
猫readyは作品条件源ではなく、本文増殖素材の密度見本。
別作品へ猫条件を移植しない。

## 3. 使用禁止・降格

- 旧8点TXTは現行テンプレ正本ではない。
- `V2_SAMPLE_NEKO_49.txt` は互換pathであり、現行49話テンプレではない。
- `assets/comparison/` は比較・再現性検証用。本文条件源ではない。
- `執筆へ渡す最小カード` は旧名。現行は `執筆凍結カード`。

## 4. STOP

現行サンプルと作品側入力が衝突する場合、作品側正規入力を優先する。
作品側入力が未読・不足・曖昧なら、サンプルで補完せずSTOP。
