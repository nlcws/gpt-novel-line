...
STATUS: OPTIONAL_REFERENCE_GATE
ACTIVE_ROUTE: none
SUPERSEDED_FOR_RUNTIME_BY: DS90 V020 PACK_CUTOUT validation
TARGET: legacy compatibility audit only

この資料は保持用の旧互換ゲートであり、現行PACK_CUTOUTへ自動差込しない。  
本文条件源・執筆入力・active required readとして扱わない。
...
- 設計さん本体: 履歴互換のため保持し、ユーザーが旧NOM監査を明示した時だけ参照する
- 話パック切り出し: 現行active routeでは使わない
- 執筆さん: writer inputへ渡さない
- 初心者公開ライン: 自動適用しない
