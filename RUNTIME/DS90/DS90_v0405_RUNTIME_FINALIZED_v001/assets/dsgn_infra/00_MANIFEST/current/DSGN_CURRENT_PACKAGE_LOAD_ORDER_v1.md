...
PURPOSE: DS90 V020へ統合済みの現行DSGNパッケージを読む順番。
...
03_REFERENCE/quality/904_小説制作絶対指針/CURRENT_READ_FIRST.md
```
...
`05_INSERT/nom/nom_gate_insert_min_v3.md` は旧互換の任意監査資料であり、active modeへ自動投入しない。
...
執筆さんへ渡すのは、梱包さんが全検査を収束させた各話の固定9ファイルだけ。  
役割は `RESTORE_SOURCE` / `RESTORE_CONSTRAINT` / `PROCESS_ONLY` /
`REFERENCE_ONLY` / `DENY_AS_BODY_SOURCE` に固定し、DSGN正本・辞書・タグ全文は渡さない。
