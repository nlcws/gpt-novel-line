...
執筆さん:
  ...
  梱包さんが収束させた各話9ファイルだけを役割別に受け取る。
  05_frozen は 01_ready〜04_crosscheck の読了代替にしない。
...
# 2. MODE SELECTION SCOPE

`DSGN.MODE` は DSGN内部のlookup・埋込・pack cutoutを起動する時だけ選ぶ。  
DS90ルート操作そのものは root `operation` が決めるため、BOOT/CHECK/CARD/LOG/MOUNT_TRANSFER等へ
DSGN.MODEを強制しない。
...
該当DSGNサブフローでMODE未選択のまま、正本・辞書・プリセットを展開しない。  
`nom_gate_insert_min_v3.md` は旧互換の任意参照であり、現行PACK_CUTOUTのactive modeではない。
