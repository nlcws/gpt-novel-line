...
LOAD_POLICY:
  ...
  - 執筆さんへはタグ辞書を渡さない。収束済み各話9ファイルだけを役割別に渡す。
...
| --- | --- | --- | --- |
| layer.runtime.v28 | layer_runtime_v28_ai_native_complete_candidate.md | current_canonical | レイヤー現行正本。ファイル名は互換固定。v27本体＋全項目意味辞書＋プリセットを内包。 |
...
| quality.904.current | 03_REFERENCE/quality/904_小説制作絶対指針/CURRENT_READ_FIRST.md | reference_bundle | 小説制作の品質思想・工程分離・熱量保持・v28現行話カード/ZIP話パック認識。CURRENT_READ_FIRSTから必要時だけ読む。 |
| nom.gate.min.v3 | nom_gate_insert_min_v3.md | optional_reference | NOM旧互換監査資料。active routeへ自動差込しない。 |
| pack.cutout.v1 | pack_cutout_module_v1.md | module | 話パック切り出しモジュール。ready/V2/layer/crosscheck/frozenを梱包。 |
| runtime.operation_conditions.v1 | DSGN_RUNTIME_OPERATION_CONDITIONS_v1.md | current_runtime_rule | DSGNサブフローの稼働条件。 |
| runtime.activation_matrix.v1 | DSGN_RUNTIME_ACTIVATION_MATRIX_v1.md | current_runtime_rule | 現行の状況別起動表。 |
| lookup.protocol.v1 | designer_lookup_protocol_v1.md | current_protocol | 該当辞書だけを引く現行lookup規約。 |
| overlay.patch.v015 | designer_runtime_overlay_patch_v015.md | retired_reference | 退役V015オーバーレイ。現行へ適用しない。 |
| overlay.manifest.v015 | runtime_overlay_manifest_v015.md | retired_reference | 退役V015 load plan。現行へ適用しない。 |
...
| runtime.load.policy | 常時読むもの・必要時だけ読むものを分ける | runtime.operation_conditions.v1<br>runtime.activation_matrix.v1 | スレッド圧迫・常時展開を避けたい時 |
| runtime.lookup.protocol | 梱包さん/設計さん間で該当辞書だけ引く | lookup.protocol.v1<br>pack.cutout.v1 | 全部読まずに一部だけ確認したい時 |
| quality.novel_absolute_principle | 条件を値引きせず工程責任を混ぜない小説制作上位品質指針 | quality.904.current | 設計品質・責任線・妥協禁止を確認する時 |
| quality.heat_delivery | 原文の熱・責任線を設計→執筆へ落とさず渡す | quality.904.current | 要約・圧縮で温度が落ちそうな時 |
| quality.card_reproducibility | 別個体でも同等精度帯へ入れるカード/レイヤー再現性 | quality.904.current | カードが薄い/説明だけ/再現性不足の時 |
...
| packager.frozen.extract | 各話9ファイル内のfrozen固定条件抽出 | pack.cutout.v1<br>layer.meaning.v1 | RESTORE_CONSTRAINTとしてhandoffする時 |
...
| nom.gate.validation | 旧NOM互換validation | nom.gate.min.v3 | 明示された旧互換監査時だけ |
| nom.reading.actual | 旧互換の実読確認 | nom.gate.min.v3 | 明示された旧互換監査時だけ |
| nom.fixed_layer | 旧互換fixed_layer確認 | nom.gate.min.v3 | 明示された旧互換監査時だけ |
| nom.heat_layer | 旧互換heat_layer確認 | nom.gate.min.v3 | 明示された旧互換監査時だけ |
| nom.forbidden_lines | 旧互換禁止線確認 | nom.gate.min.v3 | 明示された旧互換監査時だけ |
| nom.frozen_table | 旧互換frozen table | nom.gate.min.v3 | 明示された旧互換監査時だけ |
| nom.stop.no_draft | 旧互換STOP記録 | nom.gate.min.v3 | 明示された旧互換監査時だけ |
