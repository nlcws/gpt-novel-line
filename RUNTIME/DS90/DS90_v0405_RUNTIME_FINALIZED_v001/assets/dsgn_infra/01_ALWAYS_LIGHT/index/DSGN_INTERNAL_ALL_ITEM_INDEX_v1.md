...
執筆さん:
  - このインデックスは読まない。
  - 梱包さんが収束させた各話固定9ファイルだけを役割別に受け取る。
...
| --- | --- | --- | --- | --- |
| DSGN.SRC.layer.runtime.v28 | layer_runtime_v28_ai_native_complete_candidate.md | current_canonical | designer_full / packager_lookup_only | レイヤーv28現行正本。ファイル名は互換固定。全項目意味辞書とプリセットを内包。 |
...
| DSGN.SRC.quality.904.current | CURRENT_READ_FIRST.md | reference_bundle | lookup_reference | 小説制作の品質思想・工程分離・熱量保持・v28現行話カード/ZIP話パック認識。必要時のみ904 CURRENTを入口に読む。 |
| DSGN.SRC.nom.gate.min.v3 | nom_gate_insert_min_v3.md | legacy_reference_gate | optional_reference_gate | NOM旧互換監査資料。active routeなし。 |
...
| dsgn.quality.novel_absolute_principle | 小説制作で条件を値引きせず工程責任を混ぜない上位品質指針 | DSGN.SRC.quality.904.current | 設計品質・責任線・妥協禁止を確認する | lookup |
| dsgn.quality.heat_delivery | 原文の熱・責任線を設計→執筆へ落とさず渡す | DSGN.SRC.quality.904.current | 要約圧縮で熱が落ちそうな時に確認する | lookup |
| dsgn.quality.card_reproducibility | 別個体でも同等精度帯へ入れるカード/レイヤー再現性 | DSGN.SRC.quality.904.current | カードが情報だけで施工力を失う時に確認する | lookup |
| dsgn.mode.nom_gate | NOM旧互換ゲート | DSGN.SRC.nom.gate.min.v3 | 明示された互換監査時だけ参照 | optional_reference_gate |
...
| dsgn.packager.frozen.extract | frozen固定条件抽出 | DSGN.SRC.pack.cutout.v1 | 各話9ファイル内のRESTORE_CONSTRAINTとして渡す | always_light |
...
| dsgn.nom.validation_gate | NOM旧互換validation | DSGN.SRC.nom.gate.min.v3 | 明示監査時のみ。本文条件源にしない | optional_reference_gate |
| dsgn.nom.actual_reading | 旧互換の実読確認 | DSGN.SRC.nom.gate.min.v3 | 明示監査時のみ | optional_reference_gate |
| dsgn.nom.fixed_layer | 旧互換fixed_layer確認 | DSGN.SRC.nom.gate.min.v3 | 明示監査時のみ | optional_reference_gate |
| dsgn.nom.heat_layer | 旧互換heat_layer確認 | DSGN.SRC.nom.gate.min.v3 | 明示監査時のみ | optional_reference_gate |
| dsgn.nom.forbidden_lines | 旧互換禁止線確認 | DSGN.SRC.nom.gate.min.v3 | 明示監査時のみ | optional_reference_gate |
| dsgn.nom.frozen_table | 旧互換凍結条件表 | DSGN.SRC.nom.gate.min.v3 | 明示監査時のみ | optional_reference_gate |
...
梱包さんへは必要な最小応答だけ返す
↓
執筆さんへは収束済み各話9ファイルだけをwriter role付きで渡す
```
