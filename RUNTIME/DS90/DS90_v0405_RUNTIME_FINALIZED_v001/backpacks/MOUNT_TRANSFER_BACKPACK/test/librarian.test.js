...
const valid = (origin = "USER_EXPLICIT") => {
  ...
  const value = {
    ...
    transfer: {
      ...
      resultControlHandoff: { entrypoint: "00_READ_FIRST/DS90_START_GATE.md",
...
test("designer auto invocation is rejected", () => {
  const result = runMountTransferBackpack(valid("DESIGNER_AUTO"));
  assert.equal(result.decision, "STOP");
  assert.ok(result.issues.some((entry) => entry.code === "TRANSFER_USER_EXPLICIT_REQUIRED"));
});

test("runtime auto invocation is rejected", () => {
