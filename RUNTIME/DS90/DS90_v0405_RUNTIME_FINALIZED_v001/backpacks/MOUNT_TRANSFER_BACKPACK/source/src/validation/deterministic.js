...
export function validateTransferDeterministic(input) {
  ...
  const handoff = input.transfer?.resultControlHandoff;
  for (const field of ["entrypoint", "readOrder", "currentLocation", "unresolvedStops", "nextWork"]) {
    if (handoff?.[field] == null) {
      issues.push(issue("RESULT_CONTROL_HANDOFF_INCOMPLETE", `transfer.resultControlHandoff.${field}`,
        "移管後000_C control handoffの必須欄が欠けています"));
    }
  }
  const expectedEntrypoint = "00_READ_FIRST/DS90_START_GATE.md";
  const expectedMachineGate = "00_READ_FIRST/DS90_START_GATE.json";
  const expectedDispatch = "00_READ_FIRST/RUNTIME_DIRECT_DISPATCH.json";
  if (handoff?.entrypoint && handoff.entrypoint !== expectedEntrypoint) {
    issues.push(issue("RESULT_CONTROL_ENTRYPOINT_INVALID", "transfer.resultControlHandoff.entrypoint",
      `移管後再開入口は${expectedEntrypoint}である必要があります`));
  }
  if (Array.isArray(handoff?.readOrder)) {
    if (handoff.readOrder[0] !== expectedEntrypoint || !handoff.readOrder.includes(expectedMachineGate) || !handoff.readOrder.includes(expectedDispatch)) {
      issues.push(issue("RESULT_CONTROL_READ_ORDER_INVALID", "transfer.resultControlHandoff.readOrder",
        "再開readOrderはDS90 START GATEを先頭にし、machine gateとruntime dispatchを含める必要があります"));
    }
  }
  if (!allResolved(input.transfer?.restartResolvedRefs ?? [])) {
    issues.push(issue("RESTART_RESOLUTION_FAILED", "transfer.restartResolvedRefs",
      "移管後000_C control handoffから必読参照を解決・実読できません"));
  }
  return { issues };
}
