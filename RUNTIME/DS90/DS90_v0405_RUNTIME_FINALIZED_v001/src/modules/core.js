...
export const coreModule = Object.freeze({
  ...
  rules: [
    ...
    forbidden("CORE-003", "boot.useMetadataOverGate", "版メタを検証済みC/PKDB実体より優先しない"),
  ...
  transition(input) {
    if (!input.externalContext?.present) return "BOOT_READY";
    if (input.externalContext?.conflict) return "EXTERNAL_CONTEXT_STOP";
