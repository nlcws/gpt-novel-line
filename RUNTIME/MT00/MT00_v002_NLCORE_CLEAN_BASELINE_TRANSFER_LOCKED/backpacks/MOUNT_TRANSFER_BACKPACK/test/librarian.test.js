...
import { READ_ORDER, SOURCE_MANIFEST, runMountTransferBackpack } from "../src/program.js";

const digest = (value) => createHash("sha256").update(Buffer.from(value, "utf8")).digest("hex");
const index = () => ({
  items: [{ id: "MT-024-001", location: "024/VAR.md#item", shelf: "024",
  ...
  stopTags: [{ tag: "#stop:x", meaning: "x", scope: "MOUNT_TRANSFER",
    assignedTo: ["MT-024-001"], released: false }]
});
...
const valid = (origin = "USER_EXPLICIT") => {
  ...
  const value = {
    mountTransferInvocation: {
      mode: "MOUNT_TRANSFER_RUNTIME", operation: "MOUNT_TRANSFER",
  ...
  value.transfer.librarian.submissionArchive = {
    format: "ZIP",
    singleSubmissionZip: true,
    directEntryCount: 2,
    directEntries: [
      { path: "000_C.zip", type: "FILE", kind: "SHELF_ZIP" },
      { path: "022_B.zip", type: "FILE", kind: "SHELF_ZIP" }
    ],
    shelfZips: [
      {
        name: "000_C.zip",
        shelfId: "000_C",
        nestedRootEntries: [
          { path: "00_READ_FIRST", type: "DIR" },
          { path: "01_VALIDATION", type: "DIR" }
        ],
        fileEntries: [
          { path: "00_READ_FIRST/README.md", type: "FILE" },
          { path: "01_VALIDATION/REPORT.md", type: "FILE" }
        ]
      },
      {
        name: "022_B.zip",
        shelfId: "022",
        nestedRootEntries: [{ path: "022_B", type: "DIR" }],
        fileEntries: [{ path: "022_B/CORE.md", type: "FILE" }]
      }
    ],
    unclassifiedItems: [],
    newShelfProposals: [],
    claimsTransferComplete: true
  };
  return value;
};
...
test("backpack source manifest matches current root runtime files", () => {
...
test("explicit librarian backpack transfer passes and stays", () => {
  ...
  assert.equal(result.completionState, "STAY_IN_TRANSFER_RUNTIME");
});

test("runtime auto invocation returns to caller runtime", () => {
  const result = runMountTransferBackpack(valid("RUNTIME_AUTO"));
  assert.equal(result.decision, "PASS");
  assert.equal(result.completionState, "RETURN_TO_CALLER_RUNTIME");
});

test("normal non-transfer request cannot accidentally activate runtime", () => {
  const input = valid();
  delete input.mountTransferInvocation;
  const result = runMountTransferBackpack(input);
  assert.equal(result.decision, "STOP");
  assert.ok(result.issues.some((entry) => entry.code === "TRANSFER_RUNTIME_NOT_ACTIVATED"));
});
...
test("shelf ZIP packaging hard lock accepts one outer ZIP with nested folder roots", () => {
  assert.equal(runMountTransferBackpack(valid()).decision, "PASS");
});

test("outer submission ZIP rejects loose root README and folders", () => {
  const input = valid();
  input.transfer.librarian.submissionArchive.directEntryCount = 3;
  input.transfer.librarian.submissionArchive.directEntries.push(
    { path: "README.md", type: "FILE", kind: "HELPER" }
  );
  const result = runMountTransferBackpack(input);
  assert.equal(result.decision, "STOP");
  assert.ok(result.issues.some((entry) => (entry.code ?? entry.ruleId) === "SHELF_OUTER_ROOT_LOOSE_ENTRY"));
  assert.ok(result.issues.some((entry) => (entry.code ?? entry.ruleId) === "SHELF_OUTER_ENTRY_NOT_DECLARED_SHELF_ZIP"));
});

test("outer submission ZIP rejects nested folder paths at root", () => {
  const input = valid();
  input.transfer.librarian.submissionArchive.directEntries[0].path = "022/CORE.md";
  const result = runMountTransferBackpack(input);
  assert.equal(result.decision, "STOP");
  assert.ok(result.issues.some((entry) => (entry.code ?? entry.ruleId) === "SHELF_OUTER_ROOT_NESTED_PATH"));
});

test("nested shelf ZIP rejects root files", () => {
  const input = valid();
  input.transfer.librarian.submissionArchive.shelfZips[0].nestedRootEntries.push(
    { path: "README.md", type: "FILE" }
  );
  const result = runMountTransferBackpack(input);
  assert.equal(result.decision, "STOP");
  assert.ok(result.issues.some((entry) => (entry.code ?? entry.ruleId) === "SHELF_ZIP_ROOT_FILE_FORBIDDEN"));
});

test("nested shelf ZIP rejects files outside declared folders", () => {
  const input = valid();
  input.transfer.librarian.submissionArchive.shelfZips[1].fileEntries.push(
    { path: "UNDECLARED/LOOSE.md", type: "FILE" }
  );
  const result = runMountTransferBackpack(input);
  assert.equal(result.decision, "STOP");
  assert.ok(result.issues.some((entry) => (entry.code ?? entry.ruleId) === "SHELF_FILE_PARENT_FOLDER_UNDECLARED"));
});

test("unclassified items cannot claim transfer PASS and require proposals", () => {
  const input = valid();
  input.transfer.librarian.submissionArchive.unclassifiedItems.push({
    id: "U1", contentExcerpt: "unplaced condition", reason: "no existing shelf fits"
  });
  input.transfer.librarian.submissionArchive.claimsTransferComplete = true;
  const result = runMountTransferBackpack(input);
  assert.equal(result.decision, "STOP");
  assert.ok(result.issues.some((entry) => (entry.code ?? entry.ruleId) === "SHELF_UNCLASSIFIED_CANNOT_PASS"));
  assert.ok(result.issues.some((entry) => (entry.code ?? entry.ruleId) === "SHELF_UNCLASSIFIED_WITHOUT_PROPOSAL"));
});
