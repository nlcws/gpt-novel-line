# TS90 v001.25 NLCORE JSON ROUNDTRIP BOUNDARY FINALIZED

## v001.25 JSON round-trip / canonical data boundary

v001.24までの編集・診断・タイトル決定・runtime-entry規則を保持したまま、authority-bearing dataを実JSON往復可能なデータモデルへ拘束する。

- 低レベルvalidatorのPASSはruntime SUCCESS authorityではない
- Phase Bは同一sessionのPhase A証拠へ接続する
- Phase CはPhase Bで確定した最終本文とreceive時の固定条件authorityへ接続する
- runtime自身が採用タイトルを本文先頭へ挿入し、TXTDLを構築する
- final SUCCESSは最終本文SHAとartifact SHAへ拘束されたterminal audit後のみ
- STOP TXTは停止工程・理由・影響・必要修正・責任境界・保持する熱量・部分修正版の安全性を必須とする
- serialized sessionはhost内部継続用であり、敵対host向け暗号境界とは主張しない

- terminal evidence / terminalDecision SUCCESS / top-level artifact があるsessionは `state=SUCCESS` と一致必須
- serialized session validation中の例外は外へthrowせず `SESSION_INVALID` へ変換
- stable serializationは循環参照を明示拒否
- Phase B欠落など依存evidence不全時はartifact再構築を行わずfail-closed
- WAIT状態もoriginal requestからrouteを再導出して一致必須
- Phase C成立後のcontinuous routeはdeterministic artifact evidence必須
- pendingActionはschema / runtimeVersion / sessionIdまで拘束
- create / resume公開入口も循環・未処理例外を外へthrowせずSTOP相当に正規化

- `SESSION_VALID` は `JSON.stringify(session)` がthrowせず、`JSON.parse()` round-trip後も `SESSION_VALID` であることを必須化
- runtime-engine / receive-gate のstable serializerを `src/json-data.js` へ一本化
- callable `toJSON`、Date/URL/boxed/Map/Set/RegExp、BigInt、NaN/Infinity、negative zero、undefined/function/symbol値、sparse array、proxy、JSON-visible accessor、non-plain objectをauthority取得前に拒否
- depth 512 / nodes 200000 / canonical UTF-8 16MiB のhard budgetを導入
- 低レベルevaluatorもJSON境界不正をthrowせず各STOP/INVALID shapeへ正規化

詳細: `source/TS90_SKILL_RUNTIME_EVIDENCE_BINDING_LOCK_v005.md`
...
- Cモードでは、本文側の全工程が終わった段階で最終タイトルを決める
- A→C指定では、STOPが出ない限りPhase AからCモードまで連続処理し、TXTDL形式で納品する
- Cモードの採用タイトルは本文TXTの一番上に挿入する。タイトル別TXTは出さない

## v001.20 TXTDLファイル存在検査

v001.18のタイトル本文内包方針とv001.19のSTOP時混入拒否を維持したまま、TXTDLのファイルキー存在を許可集合で検査する。

- SUCCESS時の許可ファイルは `TS90_FINAL_BODY.txt` / `TS90_WORK_REPORT.txt` / optional `TS90_TITLE_CANDIDATES.txt` のみ
- STOP時の許可ファイルは `TS90_STOP_REPORT.txt` のみ
- `TS90_FINAL_TITLE.txt` は空でも存在した時点でFAIL
- SUCCESS時に `TS90_STOP_REPORT.txt` や `EXTRA.txt` が混ざればFAIL
- STOP時に `TS90_FINAL_BODY.txt` / `TS90_WORK_REPORT.txt` / `EXTRA.txt` が混ざれば、空でもFAIL

## v001.19 TXTDL実装整合

v001.18のタイトル本文内包方針を維持したまま、実装側の取り残しを修正した。

- 成功時の戻り値 `files` は `TS90_FINAL_BODY.txt` と `TS90_WORK_REPORT.txt` を基本とする
- `TS90_FINAL_TITLE.txt` は成功時もSTOP時も出さない
- STOP時は `TS90_STOP_REPORT.txt` のみ納品し、`TS90_FINAL_BODY.txt` / `TS90_WORK_REPORT.txt` が混ざればFAIL

## v001.18 採用タイトルの本文先頭挿入

Cモードが候補から選んだ `話タイトル` を、`TS90_FINAL_BODY.txt` の先頭タイトルブロックに入れる。
...
作品タイトルと部・編等は未提示なら作らない。
話数は対象範囲・話番号・ユーザー指定から確認できるものを使う。
話タイトルは、Cモードの `採用タイトル` と完全一致させる。
順序が違う場合、または話数・話タイトルが本文TXTの先頭ブロックにない場合はFAIL。
タイトル候補・採用理由・却下候補は `TS90_WORK_REPORT.txt` 側へ残す。
`TS90_FINAL_TITLE.txt` は出さない。

詳細: `source/TS90_A_TO_C_TXTDL_OUTPUT_LOCK_v002.md`

## v001.17 A→C連続TXTDL納品

本文が読めて実行意図がある場合、通常のTS90納品は以下を一回で通す。
...
TS90_TITLE_CANDIDATES.txt  # optional
```
...
本文側の作業完了後に、最終タイトルを決めるCモードを追加した。
...
- Cモードは本文を修正しない
- 新事件、新設定、新関係、新事実を足さない
- 最終本文と固定条件からタイトル候補を出す
- 採用タイトルを1つ決め、採用理由と却下理由を分ける
- 未提示のシリーズ全体タイトル規則を確認済み扱いしない
- 最終本文が未確定ならタイトル決定せずSTOPする

詳細: `source/TS90_PHASE_C_TITLE_DECISION_LOCK_v001.md`
...
## v001.22 terminal serialized-session authority

- SUCCESS is reconstructed and revalidated from request + stored Phase A/B/C/artifact/terminal evidence.
- `packageVersion` and deterministic `sessionId` are validation-bound.
- terminal gate, T05 action binding and terminal packet SHA are rechecked on every serialized-session validation.
- This remains a host-internal non-cryptographic boundary; no claim is made against a malicious host capable of arbitrary session recomputation.
