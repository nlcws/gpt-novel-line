...
for (const expected of TITLE_DECISION_LOCK_MANIFEST) {
  test(`${expected.path} title decision lock source is byte-identical`, () => {
    const url = new URL(`../${expected.path}`, import.meta.url);
    const content = readFileSync(url);
    assert.equal(statSync(url).size, expected.bytes);
    assert.equal(createHash("sha256").update(content).digest("hex").toUpperCase(), expected.sha256);
    const text = content.toString("utf8");
    assert.ok(text.includes("Phase C"));
    assert.ok(text.includes("採用タイトル"));
    assert.ok(text.includes("body text"));
  });
}

for (const expected of A_TO_C_TXTDL_LOCK_MANIFEST) {
  test(`${expected.path} A-to-C TXTDL lock source is byte-identical`, () => {
    const url = new URL(`../${expected.path}`, import.meta.url);
    const content = readFileSync(url);
    assert.equal(statSync(url).size, expected.bytes);
    assert.equal(createHash("sha256").update(content).digest("hex").toUpperCase(), expected.sha256);
    const text = content.toString("utf8");
    assert.ok(text.includes("A-to-C"));
    assert.ok(text.includes("TS90_FINAL_BODY.txt"));
    assert.ok(text.includes("Do not output a separate `TS90_FINAL_TITLE.txt`"));
    assert.ok(text.includes("work title / 作品タイトル"));
    assert.ok(text.includes("episode number or target label / 話数"));
    assert.ok(text.includes("TS90_STOP_REPORT.txt"));
  });
}

test("chat mount auto boots receive gate and terminal locks but never auto repairs", () => {
  assert.equal(RUNTIME_VERSION, "ts90-v001.25-nlcore-json-roundtrip-boundary");
  assert.equal(PACKAGE_VERSION, "TS90_v001_25_NLCORE_JSON_ROUNDTRIP_BOUNDARY_FINALIZED");
...
test("source read order remains only original files while boot read order includes terminal locks", () => {
  ...
  for (const lockPath of SKILL_RUNTIME_EVIDENCE_LOCK_READ_ORDER) assert.ok(BOOT_READ_ORDER.includes(lockPath));
});
...
test("package expected files includes lock sources and package verifier without adding unknown files", () => {
  ...
  assert.ok(PACKAGE_EXPECTED_FILES.includes("source/TS90_SKILL_RUNTIME_EVIDENCE_BINDING_LOCK_v005.md"));
  assert.ok(PACKAGE_EXPECTED_FILES.includes("src/runtime-engine.js"));
...
test("adaptive constants preserve diagnosis, finalization, and rollback policy", () => {
  ...
  assert.ok(ADAPTIVE_FINALIZATION_ORDER.includes("Cモードタイトル決定"));
  assert.ok(ADAPTIVE_FINALIZATION_ORDER.includes("TXTDL納品"));
  ...
  assert.deepEqual(TITLE_DECISION_LOCK_READ_ORDER, ["source/TS90_PHASE_C_TITLE_DECISION_LOCK_v001.md"]);
  assert.deepEqual(A_TO_C_TXTDL_LOCK_READ_ORDER, ["source/TS90_A_TO_C_TXTDL_OUTPUT_LOCK_v002.md"]);
  assert.equal(TITLE_DECISION_POLICY.mode, "PHASE_C_TITLE_DECISION");
  assert.equal(TITLE_DECISION_POLICY.bodyRevisionDenied, true);
  assert.ok(TITLE_DECISION_REQUIRED_FIELDS.includes("採用タイトル"));
  assert.equal(A_TO_C_TXTDL_POLICY.defaultRun, "PHASE_A_TO_C_UNLESS_STOP");
  assert.equal(A_TO_C_TXTDL_POLICY.perStageUserNextRequired, false);
  assert.equal(A_TO_C_TXTDL_POLICY.titleBlockInsertedAtBodyTop, true);
  assert.deepEqual(A_TO_C_TXTDL_POLICY.titleBlockOrder, ["作品タイトル?", "部・編等?", "話数", "話タイトル"]);
  assert.equal(A_TO_C_TXTDL_POLICY.selectedTitleMustBeInTitleBlock, true);
  assert.equal(A_TO_C_TXTDL_POLICY.episodeLabelRequiredInTitleBlock, true);
  assert.equal(A_TO_C_TXTDL_POLICY.separateFinalTitleTxtDenied, true);
  assert.deepEqual(A_TO_C_TXTDL_SUCCESS_FILES, ["TS90_FINAL_BODY.txt", "TS90_WORK_REPORT.txt"]);
  assert.equal(A_TO_C_TXTDL_STAGE_ORDER.at(-1), "TXTDL納品");
});

test("Phase C title decision passes only after body completion", () => {
  const report = {
    対象範囲: "P1F-001",
    本文状態: "最終冷却・固定条件照合済み",
    固定条件: "雨上がりの駅で、約束を変えずに終える",
    タイトル候補: [
      { title: "雨上がりの約束", reason: "本文の場面と核に一致" },
      { title: "駅で待つ人", reason: "場所と人物の待機を拾う" }
    ],
    採用タイトル: "雨上がりの約束",
    採用理由: "本文の天候、小道具、着地点を過不足なく拾う",
    却下候補と理由: [
      { title: "世界を変えた日", reason: "本文にない規模を示す" }
    ],
  ...
  const result = evaluateTitleDecisionReport(report, {
    ...
    finalBodyText: "雨上がりの駅で、ふたりは約束を変えずに立っていた。",
    fixedConditions: "雨上がりの駅で、約束を変えずに終える"
  });
  assert.equal(result.decision, "TITLE_DECISION_PASS");
  assert.equal(result.selectedTitle, "雨上がりの約束");

  const unfinished = evaluateTitleDecisionReport(report, {
    bodyWorkComplete: false,
    finalBodyText: "雨上がりの駅で、ふたりは約束を変えずに立っていた。",
    fixedConditions: "雨上がりの駅で、約束を変えずに終える"
  });
  assert.equal(unfinished.decision, "TITLE_DECISION_STOP");
  assert.ok(unfinished.failures.some((entry) => entry.code === "BODY_WORK_NOT_COMPLETE"));

  const falseClaim = evaluateTitleDecisionReport({ ...report, newStoryFactAdded: true }, {
    bodyWorkComplete: true,
    finalBodyText: "雨上がりの駅で、ふたりは約束を変えずに立っていた。",
    fixedConditions: "雨上がりの駅で、約束を変えずに終える"
  });
  assert.equal(falseClaim.decision, "TITLE_DECISION_STOP");
  assert.ok(falseClaim.failures.some((entry) => entry.code === "PHASE_C_NEW_STORY_FACT_DENIED"));
});

test("A-to-C TXTDL output passes after final body and stops safely on STOP", () => {
  const phaseCTitleDecision = {
    対象範囲: "P1F-001",
    本文状態: "最終冷却・固定条件照合済み",
    固定条件: "雨上がりの駅で、約束を変えずに終える",
    タイトル候補: [
  ...
  const packet = {
    ...
    fixedConditions: "雨上がりの駅で、約束を変えずに終える",
    TXTDL: {
      "TS90_FINAL_BODY.txt": "契の外\n第1部 表\nP1F-001\n雨上がりの約束\n\n雨上がりの駅で、ふたりは約束を変えずに立っていた。",
      "TS90_WORK_REPORT.txt": "Phase A診断。Phase B修正。冷却PASS。固定条件照合PASS。作品タイトル: 契の外。部表記: 第1部 表。話数: P1F-001。候補: 雨上がりの約束（本文の場面と核に一致） / 駅で待つ人（場所と人物の待機を拾う）。採用タイトル: 雨上がりの約束。採用理由: 本文の天候、小道具、着地点を過不足なく拾う。却下: 世界を変えた日（本文にない規模を示す）。未確認 / 保留: シリーズ全体の表題規則は未提示。"
    },
    phaseCTitleDecision,
    終端ゲート: terminalGate()
  };
  const result = evaluateAToCTXTDLOutput(packet);
  assert.equal(result.decision, "A_TO_C_TXTDL_PASS");
  assert.deepEqual(result.files, ["TS90_FINAL_BODY.txt", "TS90_WORK_REPORT.txt"]);
  assert.ok(!result.files.includes("TS90_FINAL_TITLE.txt"));

  const withOptionalCandidates = evaluateAToCTXTDLOutput({
    ...packet,
    TXTDL: {
      ...packet.TXTDL,
      "TS90_TITLE_CANDIDATES.txt": "雨上がりの約束\n駅で待つ人"
    }
  });
  assert.equal(withOptionalCandidates.decision, "A_TO_C_TXTDL_PASS");
  assert.deepEqual(withOptionalCandidates.files, ["TS90_FINAL_BODY.txt", "TS90_WORK_REPORT.txt", "TS90_TITLE_CANDIDATES.txt"]);

  const mixed = evaluateAToCTXTDLOutput({
    ...packet,
    TXTDL: {
      ...packet.TXTDL,
      "TS90_FINAL_BODY.txt": `雨上がりの駅で、ふたりは約束を変えずに立っていた。\n${packet.TXTDL["TS90_WORK_REPORT.txt"]}`
    }
  });
  assert.equal(mixed.decision, "A_TO_C_TXTDL_STOP");
  assert.ok(mixed.failures.some((entry) => entry.code === "WORK_REPORT_MIXED_INTO_BODY_DENIED"));

  const missingBodyTitle = evaluateAToCTXTDLOutput({
    ...packet,
    TXTDL: {
      ...packet.TXTDL,
      "TS90_FINAL_BODY.txt": "雨上がりの駅で、ふたりは約束を変えずに立っていた。"
  ...
  assert.equal(missingBodyTitle.decision, "A_TO_C_TXTDL_STOP");
  assert.ok(missingBodyTitle.failures.some((entry) => entry.code === "TITLE_BLOCK_LINE_MISMATCH"));

  const separateTitle = evaluateAToCTXTDLOutput({
    ...packet,
    TXTDL: {
      ...packet.TXTDL,
      "TS90_FINAL_TITLE.txt": "雨上がりの約束"
    }
  });
  assert.equal(separateTitle.decision, "A_TO_C_TXTDL_STOP");
  assert.ok(separateTitle.failures.some((entry) => entry.code === "SEPARATE_FINAL_TITLE_TXT_DENIED"));

  const emptySeparateTitle = evaluateAToCTXTDLOutput({
    ...packet,
    TXTDL: {
      ...packet.TXTDL,
      "TS90_FINAL_TITLE.txt": ""
    }
  });
  assert.equal(emptySeparateTitle.decision, "A_TO_C_TXTDL_STOP");
  assert.ok(emptySeparateTitle.failures.some((entry) => entry.code === "SEPARATE_FINAL_TITLE_TXT_DENIED"));

  const successWithStopReport = evaluateAToCTXTDLOutput({
    ...packet,
    TXTDL: {
      ...packet.TXTDL,
  ...
  assert.equal(successWithStopReport.decision, "A_TO_C_TXTDL_STOP");
  assert.ok(successWithStopReport.failures.some((entry) => entry.code === "TXTDL_FILE_NOT_ALLOWED_ON_SUCCESS"));

  const successWithExtra = evaluateAToCTXTDLOutput({
    ...packet,
    TXTDL: {
      ...packet.TXTDL,
      "EXTRA.txt": ""
  ...
  assert.equal(successWithExtra.decision, "A_TO_C_TXTDL_STOP");
  assert.ok(successWithExtra.failures.some((entry) => entry.code === "TXTDL_FILE_NOT_ALLOWED_ON_SUCCESS"));

  const stopPacket = evaluateAToCTXTDLOutput({
    status: "STOP",
    stoppedStage: "固定条件照合",
    reason: "固定核と本文の着地が一致しない",
    TXTDL: {
      "TS90_STOP_REPORT.txt": "停止工程: 固定条件照合\n理由: 固定核と本文の着地が一致しない\n影響: 安全な最終本文を確定できない\n必要修正: 本文着地の確認\n責任境界: 設計側の固定核確認が必要\n保持する熱量: 約束を変えずに終える核\n部分修正版の安全性: 使用不可"
    }
  });
  assert.equal(stopPacket.decision, "A_TO_C_TXTDL_STOP_PACKET");

  const falseStopTitle = evaluateAToCTXTDLOutput({
    status: "STOP",
    stoppedStage: "固定条件照合",
    reason: "固定核と本文の着地が一致しない",
    TXTDL: {
      "TS90_STOP_REPORT.txt": "停止工程: 固定条件照合",
      "TS90_FINAL_TITLE.txt": "仮タイトル"
    }
  });
  assert.equal(falseStopTitle.decision, "A_TO_C_TXTDL_STOP");
  assert.ok(falseStopTitle.failures.some((entry) => entry.code === "FINAL_TITLE_DENIED_ON_STOP"));

  const falseStopEmptyTitle = evaluateAToCTXTDLOutput({
    status: "STOP",
    stoppedStage: "固定条件照合",
    reason: "固定核と本文の着地が一致しない",
    TXTDL: {
  ...
  assert.equal(falseStopEmptyTitle.decision, "A_TO_C_TXTDL_STOP");
  assert.ok(falseStopEmptyTitle.failures.some((entry) => entry.code === "FINAL_TITLE_DENIED_ON_STOP"));

  const falseStopPayloads = evaluateAToCTXTDLOutput({
    status: "STOP",
    stoppedStage: "固定条件照合",
    reason: "固定核と本文の着地が一致しない",
    TXTDL: {
      "TS90_STOP_REPORT.txt": "停止工程: 固定条件照合",
      "TS90_FINAL_BODY.txt": "契の外\n第1部 表\nP1F-001\n仮タイトル\n\n本文",
      "TS90_WORK_REPORT.txt": "途中報告"
    }
  });
  assert.equal(falseStopPayloads.decision, "A_TO_C_TXTDL_STOP");
  assert.ok(falseStopPayloads.failures.some((entry) => entry.code === "FINAL_BODY_DENIED_ON_STOP"));
  assert.ok(falseStopPayloads.failures.some((entry) => entry.code === "WORK_REPORT_DENIED_ON_STOP"));

  const falseStopEmptyPayloads = evaluateAToCTXTDLOutput({
    status: "STOP",
    stoppedStage: "固定条件照合",
    reason: "固定核と本文の着地が一致しない",
    TXTDL: {
  ...
  assert.equal(falseStopEmptyPayloads.decision, "A_TO_C_TXTDL_STOP");
  assert.ok(falseStopEmptyPayloads.failures.some((entry) => entry.code === "FINAL_BODY_DENIED_ON_STOP"));
  assert.ok(falseStopEmptyPayloads.failures.some((entry) => entry.code === "WORK_REPORT_DENIED_ON_STOP"));

  const falseStopExtra = evaluateAToCTXTDLOutput({
    status: "STOP",
    stoppedStage: "固定条件照合",
    reason: "固定核と本文の着地が一致しない",
    TXTDL: {
  ...
  assert.equal(falseStopExtra.decision, "A_TO_C_TXTDL_STOP");
  assert.ok(falseStopExtra.failures.some((entry) => entry.code === "TXTDL_FILE_NOT_ALLOWED_ON_STOP"));
});
