...
import {
  ...
  REVISION_STRENGTHS,
  A_TO_C_TXTDL_OPTIONAL_FILES,
  A_TO_C_TXTDL_POLICY,
  A_TO_C_TXTDL_STOP_FILES,
  A_TO_C_TXTDL_SUCCESS_FILES,
  TITLE_DECISION_POLICY,
  TITLE_DECISION_REQUIRED_FIELDS,
...
const firstNonEmptyLine = (value) => typeof value === "string"
  ? (value.split(/\r?\n/).map((line) => line.trim()).find(hasText) ?? "")
  : "";
const nonEmptyLines = (value) => typeof value === "string"
  ? value.split(/\r?\n/).map((line) => line.trim()).filter(hasText)
  : [];
const txdlFileKeys = (container) => isObject(container)
  ? Object.keys(container).filter((key) => /\.txt$/i.test(key))
  : [];
const A_TO_C_TXTDL_SUCCESS_ALLOWED_FILES = Object.freeze([
  ...A_TO_C_TXTDL_SUCCESS_FILES,
  ...A_TO_C_TXTDL_OPTIONAL_FILES
]);

function pushDisallowedTXTDLFileFailure(failures, key, mode) {
  if (mode === "STOP") {
    if (key === "TS90_FINAL_BODY.txt") failures.push(fail("FINAL_BODY_DENIED_ON_STOP", key));
    else if (key === "TS90_WORK_REPORT.txt") failures.push(fail("WORK_REPORT_DENIED_ON_STOP", key));
    else if (key === "TS90_FINAL_TITLE.txt") failures.push(fail("FINAL_TITLE_DENIED_ON_STOP", key));
    else failures.push(fail("TXTDL_FILE_NOT_ALLOWED_ON_STOP", key));
    return;
  }
  if (key === "TS90_FINAL_TITLE.txt") failures.push(fail("SEPARATE_FINAL_TITLE_TXT_DENIED", key));
  else failures.push(fail("TXTDL_FILE_NOT_ALLOWED_ON_SUCCESS", key));
}
...
function jsonBoundaryStop(values, decision, path = "jsonBoundary") {
  try {
    for (const value of values) stableStringify(value);
    return null;
  } catch (error) {
    const code = jsonDataFailureCode(error);
    return Object.freeze({
      decision,
      failures: Object.freeze([fail(`JSON_DATA_BOUNDARY_DENIED:${code}`, path)])
    });
  }
}
...
export function evaluateAdaptiveDiagnosis(diagnosis = {}) {
  const boundary = jsonBoundaryStop([diagnosis], "DIAGNOSIS_INVALID", "diagnosis");
...
export function evaluateAdaptiveEditPlan(input = {}) {
  const boundary = jsonBoundaryStop([input], "PLAN_INVALID", "plan");
  if (boundary) return boundary;
...
export function evaluateRevisionComparison(comparison = {}, options = {}) {
  const boundary = jsonBoundaryStop([comparison, options], "COMPARISON_STOP", "comparison");
  if (boundary) return Object.freeze({ ...boundary, userChoiceRequired: false });
  const failures = [];
...
export function evaluateAdaptiveEditorReport(report = {}, context = {}) {
  const boundary = jsonBoundaryStop([report, context], "ADAPTIVE_REPORT_STOP", "編集主任レポート");
  if (boundary) return boundary;
  const failures = [];
  ...
  const planInput = { plan: report.plan, stopSignalsDetected: activeStopSignals };
  if (hasText(report.allStagesStrongJustification)) planInput.allStagesStrongJustification = report.allStagesStrongJustification;
  const planResult = evaluateAdaptiveEditPlan(planInput);
  ...
  const normalizedReportedDiff = Array.isArray(report.diffEvidence) ? report.diffEvidence : null;
  const normalizedActualDiff = actualDiffHunks;
...
export function evaluateTerminalLocks(output = {}, { allowStopReport = false } = {}) {
  const boundary = jsonBoundaryStop([output, { allowStopReport }], "TERMINAL_LOCKS_STOP", "終端ゲート");
  if (boundary) return boundary;
  const failures = [];
...
export function evaluateToshiReceive(input = {}) {
  const boundary = jsonBoundaryStop([input], "RECEIVE_STOP", "input");
  if (boundary) return Object.freeze({ ...boundary, phase: null });
  const failures = [];
...
export function evaluateTitleDecisionReport(report = {}, context = {}) {
  const boundary = jsonBoundaryStop([report, context], "TITLE_DECISION_STOP", "titleDecision");
  if (boundary) return Object.freeze({ ...boundary, selectedTitle: "" });
  const failures = [];
  if (!isObject(report)) {
    return Object.freeze({ decision: "TITLE_DECISION_STOP", failures: Object.freeze([fail("TITLE_DECISION_REPORT_REQUIRED", "Cモード タイトル決定")]) });
  }

  const bodyText = normalizeBodyText(context.finalBodyText ?? context.revisedBodyText ?? context.acceptedBodyText ?? "");
  if (TITLE_DECISION_POLICY.finalBodyRequired && !hasText(bodyText)) failures.push(fail("FINAL_BODY_TEXT_REQUIRED", "context.finalBodyText"));
  if (context.bodyWorkComplete !== true) failures.push(fail("BODY_WORK_NOT_COMPLETE", "context.bodyWorkComplete"));

  for (const field of TITLE_DECISION_REQUIRED_FIELDS) {
    const value = report[field] ?? report[field.replaceAll(" ", "_")];
    if (field === "タイトル候補" || field === "却下候補と理由") {
      if (!Array.isArray(value) || value.length === 0) failures.push(fail("TITLE_DECISION_FIELD_ARRAY_REQUIRED", field));
    } else if (!hasText(value)) {
      failures.push(fail("TITLE_DECISION_FIELD_REQUIRED", field));
    }
  }

  const contextFixedCore = firstPresentText(context.fixedConditions, context.preservedCore);
  const reportFixedCore = firstPresentText(report["固定条件"], report.fixedConditions);
  const fixedCore = contextFixedCore ?? reportFixedCore;
  if (TITLE_DECISION_POLICY.fixedCoreRequired && !hasText(fixedCore)) failures.push(fail("FIXED_CORE_REQUIRED_FOR_TITLE", "固定条件"));
  if (hasText(contextFixedCore) && hasText(reportFixedCore) && normalizeBodyText(contextFixedCore) !== normalizeBodyText(reportFixedCore)) {
    failures.push(fail("TITLE_FIXED_CORE_MISMATCH", "固定条件"));
  }
  if (report.bodyRevisionPerformedInPhaseC === true) failures.push(fail("PHASE_C_BODY_REVISION_DENIED", "bodyRevisionPerformedInPhaseC"));
  if (report.newStoryFactAdded === true) failures.push(fail("PHASE_C_NEW_STORY_FACT_DENIED", "newStoryFactAdded"));
  if (report.unprovidedSeriesRuleChecked === true) failures.push(fail("UNPROVIDED_SERIES_RULE_CLAIM_DENIED", "unprovidedSeriesRuleChecked"));

  const selectedTitle = textValue(report["採用タイトル"] ?? report.selectedTitle);
  ...
  if (hasText(selectedTitle)) {
    const candidateTitles = candidates.map((item) => isObject(item) ? textValue(item.title ?? item.タイトル) : textValue(item)).filter(hasText);
    if (candidateTitles.length > 0 && !candidateTitles.includes(selectedTitle)) failures.push(fail("SELECTED_TITLE_NOT_IN_CANDIDATES", "採用タイトル"));
  }
  for (const [index, item] of candidates.entries()) {
    if (isObject(item)) {
      if (!hasText(item.title ?? item.タイトル)) failures.push(fail("TITLE_CANDIDATE_TITLE_REQUIRED", `タイトル候補[${index}]`));
      if (!hasText(item.reason ?? item.理由)) failures.push(fail("TITLE_CANDIDATE_REASON_REQUIRED", `タイトル候補[${index}]`));
      if (item.dependsOnUnprovidedMaterial === true) failures.push(fail("TITLE_CANDIDATE_UNPROVIDED_MATERIAL_DENIED", `タイトル候補[${index}]`));
      if (item.misleading === true) failures.push(fail("TITLE_CANDIDATE_MISLEADING_DENIED", `タイトル候補[${index}]`));
      if (item.newFact === true) failures.push(fail("TITLE_CANDIDATE_NEW_FACT_DENIED", `タイトル候補[${index}]`));
    } else if (!hasText(item)) {
      failures.push(fail("TITLE_CANDIDATE_TITLE_REQUIRED", `タイトル候補[${index}]`));
    }
  }
  for (const [index, item] of rejected.entries()) {
    if (isObject(item)) {
      if (!hasText(item.title ?? item.タイトル) || !hasText(item.reason ?? item.理由)) failures.push(fail("REJECTED_TITLE_REASON_REQUIRED", `却下候補と理由[${index}]`));
    } else if (!hasText(item)) {
      failures.push(fail("REJECTED_TITLE_REASON_REQUIRED", `却下候補と理由[${index}]`));
    }
  }

  if (context.requireSelectedTitleInBody === true && hasText(selectedTitle) && !bodyText.includes(selectedTitle)) {
    failures.push(fail("SELECTED_TITLE_NOT_VERBATIM_IN_BODY", "採用タイトル"));
  }

  return Object.freeze({
    decision: failures.length === 0 ? "TITLE_DECISION_PASS" : "TITLE_DECISION_STOP",
    selectedTitle,
    failures: Object.freeze(failures)
  });
}

function workReportBindingFailures(workReportTxt, titleReport = {}, packet = {}, context = {}) {
  const failures = [];
  if (!hasText(workReportTxt)) return failures;
  const requiredLiterals = ["Phase A", "Phase B", "冷却", "固定条件"];
  for (const literal of requiredLiterals) {
    if (!workReportTxt.includes(literal)) failures.push(fail("WORK_REPORT_REQUIRED_SECTION_MISSING", `TS90_WORK_REPORT.txt:${literal}`));
  }
  const selectedTitle = firstPresentText(titleReport["採用タイトル"], titleReport.selectedTitle);
  const selectedReason = firstPresentText(titleReport["採用理由"], titleReport.selectedReason);
  const remaining = firstPresentText(titleReport["未確認 / 保留"], titleReport.remainingConcerns);
  for (const [label, value] of [["selectedTitle", selectedTitle], ["selectedReason", selectedReason], ["remainingConcerns", remaining]]) {
    if (hasText(value) && !workReportTxt.includes(value)) failures.push(fail("WORK_REPORT_TITLE_EVIDENCE_MISSING", `TS90_WORK_REPORT.txt:${label}`));
  }
  const candidates = Array.isArray(titleReport["タイトル候補"] ?? titleReport.titleCandidates) ? (titleReport["タイトル候補"] ?? titleReport.titleCandidates) : [];
  for (const [index, item] of candidates.entries()) {
    const title = isObject(item) ? firstPresentText(item.title, item.タイトル) : textValue(item);
    const reason = isObject(item) ? firstPresentText(item.reason, item.理由) : null;
    if (hasText(title) && !workReportTxt.includes(title)) failures.push(fail("WORK_REPORT_CANDIDATE_MISSING", `TS90_WORK_REPORT.txt:候補[${index}]`));
    if (hasText(reason) && !workReportTxt.includes(reason)) failures.push(fail("WORK_REPORT_CANDIDATE_REASON_MISSING", `TS90_WORK_REPORT.txt:候補理由[${index}]`));
  }
  const rejected = Array.isArray(titleReport["却下候補と理由"] ?? titleReport.rejectedTitles) ? (titleReport["却下候補と理由"] ?? titleReport.rejectedTitles) : [];
  for (const [index, item] of rejected.entries()) {
    const title = isObject(item) ? firstPresentText(item.title, item.タイトル) : textValue(item);
    const reason = isObject(item) ? firstPresentText(item.reason, item.理由) : null;
    if (hasText(title) && !workReportTxt.includes(title)) failures.push(fail("WORK_REPORT_REJECTED_TITLE_MISSING", `TS90_WORK_REPORT.txt:却下[${index}]`));
    if (hasText(reason) && !workReportTxt.includes(reason)) failures.push(fail("WORK_REPORT_REJECTED_REASON_MISSING", `TS90_WORK_REPORT.txt:却下理由[${index}]`));
  }
  const workTitle = firstPresentText(packet.workTitle, packet["作品タイトル"], context.workTitle, context["作品タイトル"]);
  const partLabel = firstPresentText(packet.partLabel, packet["部表記"], packet["編表記"], packet["部・編等"], context.partLabel, context["部表記"], context["編表記"], context["部・編等"]);
  const episodeLabel = firstPresentText(packet.episodeLabel, packet.storyNumberLabel, packet["話数"], packet["話番号"], packet.targetRange, titleReport["対象範囲"], context.episodeLabel, context.storyNumberLabel, context["話数"], context["話番号"], context.targetRange);
  for (const [label, value] of [["workTitle", workTitle], ["partLabel", partLabel], ["episodeLabel", episodeLabel]]) {
    if (hasText(value) && !workReportTxt.includes(value)) failures.push(fail("WORK_REPORT_LABEL_EVIDENCE_MISSING", `TS90_WORK_REPORT.txt:${label}`));
  }
  return failures;
}

function stopReportBindingFailures(stopReport, stoppedStage, reason) {
  const failures = [];
  const requiredTokens = [
    ["STOP_REPORT_IMPACT_REQUIRED", /影響\s*[:：]/],
    ["STOP_REPORT_REQUIRED_REPAIR_REQUIRED", /必要修正|必要材料|不足材料/],
    ["STOP_REPORT_RESPONSIBILITY_BOUNDARY_REQUIRED", /責任境界\s*[:：]/],
    ["STOP_REPORT_PRESERVED_HEAT_REQUIRED", /保持する熱量|保持する核/],
    ["STOP_REPORT_PARTIAL_BODY_SAFETY_REQUIRED", /部分修正版|partial revised body|途中本文.*安全/i]
  ];
  if (hasText(stoppedStage) && !stopReport.includes(stoppedStage)) failures.push(fail("STOP_REPORT_STAGE_NOT_BOUND", "TS90_STOP_REPORT.txt"));
  if (hasText(reason) && !stopReport.includes(reason)) failures.push(fail("STOP_REPORT_REASON_NOT_BOUND", "TS90_STOP_REPORT.txt"));
  for (const [code, pattern] of requiredTokens) {
    if (!pattern.test(stopReport)) failures.push(fail(code, "TS90_STOP_REPORT.txt"));
  }
  return failures;
}

export function evaluateAToCTXTDLOutput(packet = {}, context = {}) {
  const boundary = jsonBoundaryStop([packet, context], "A_TO_C_TXTDL_STOP", "TXTDL");
  if (boundary) return boundary;
  const failures = [];
  if (!isObject(packet)) {
    return Object.freeze({ decision: "A_TO_C_TXTDL_STOP", failures: Object.freeze([fail("A_TO_C_TXTDL_PACKET_REQUIRED", "TXTDL")]) });
  }

  const container = isObject(packet.TXTDL) ? packet.TXTDL : packet;
  const stopped = packet.stopped === true || packet.stop === true || packet.status === "STOP" || packet.decision === "STOP";
  const readText = (...keys) => firstPresentText(...keys.map((key) => container[key] ?? packet[key]));
  const fileKeys = txdlFileKeys(container);
  const fileKeySet = new Set(fileKeys);

  if (stopped) {
    const stopReport = readText("TS90_STOP_REPORT.txt", "stopReportTxt", "STOP報告TXT");
    const stoppedStage = firstPresentText(packet.stoppedStage, packet["停止工程"], container.stoppedStage, container["停止工程"]);
    const reason = firstPresentText(packet.reason, packet["理由"], container.reason, container["理由"]);
    for (const key of fileKeys) {
      if (!A_TO_C_TXTDL_STOP_FILES.includes(key)) pushDisallowedTXTDLFileFailure(failures, key, "STOP");
    }
    if (!fileKeySet.has("TS90_STOP_REPORT.txt") || !hasText(stopReport)) failures.push(fail("STOP_REPORT_TXT_REQUIRED", "TS90_STOP_REPORT.txt"));
    if (!hasText(stoppedStage)) failures.push(fail("STOP_STAGE_REQUIRED", "stoppedStage"));
    if (!hasText(reason)) failures.push(fail("STOP_REASON_REQUIRED", "reason"));
    if (hasText(stopReport)) failures.push(...stopReportBindingFailures(stopReport, stoppedStage, reason));
    return Object.freeze({
      decision: failures.length === 0 ? "A_TO_C_TXTDL_STOP_PACKET" : "A_TO_C_TXTDL_STOP",
      stoppedStage,
      failures: Object.freeze(failures)
    });
  }

  if (packet.perStageUserNextRequired === true) failures.push(fail("PER_STAGE_NEXT_REQUIRED_DENIED", "perStageUserNextRequired"));
  if (packet.bodyReportSeparated !== true) failures.push(fail("BODY_REPORT_SEPARATION_REQUIRED", "bodyReportSeparated"));
  if ((packet.finalCoolingComplete ?? context.finalCoolingComplete) !== true) failures.push(fail("FINAL_COOLING_REQUIRED", "finalCoolingComplete"));
  if ((packet.fixedConditionCheckPassed ?? context.fixedConditionCheckPassed) !== true) failures.push(fail("FIXED_CONDITION_CHECK_REQUIRED", "fixedConditionCheckPassed"));
  const terminalResult = evaluateTerminalLocks(packet);
  failures.push(...terminalResult.failures);

  const finalBodyTxt = readText("TS90_FINAL_BODY.txt", "finalBodyTxt", "本文TXT");
  const workReportTxt = readText("TS90_WORK_REPORT.txt", "workReportTxt", "作業報告TXT");
  for (const key of fileKeys) {
    if (!A_TO_C_TXTDL_SUCCESS_ALLOWED_FILES.includes(key)) pushDisallowedTXTDLFileFailure(failures, key, "SUCCESS");
  }
  if (A_TO_C_TXTDL_POLICY.finalBodyTxtRequired && (!fileKeySet.has("TS90_FINAL_BODY.txt") || !hasText(finalBodyTxt))) failures.push(fail("FINAL_BODY_TXT_REQUIRED", "TS90_FINAL_BODY.txt"));
  if (A_TO_C_TXTDL_POLICY.workReportTxtRequired && (!fileKeySet.has("TS90_WORK_REPORT.txt") || !hasText(workReportTxt))) failures.push(fail("WORK_REPORT_TXT_REQUIRED", "TS90_WORK_REPORT.txt"));
  if (hasText(workReportTxt) && hasText(finalBodyTxt) && finalBodyTxt.includes(workReportTxt)) {
    failures.push(fail("WORK_REPORT_MIXED_INTO_BODY_DENIED", "TS90_FINAL_BODY.txt"));
  }

  const titleReport = packet.phaseCTitleDecision ?? packet["Cモードタイトル決定"] ?? container.phaseCTitleDecision ?? container["Cモードタイトル決定"];
  if (!isObject(titleReport)) {
    failures.push(fail("PHASE_C_TITLE_REPORT_REQUIRED", "phaseCTitleDecision"));
  } else {
    const titleResult = evaluateTitleDecisionReport(titleReport, {
      bodyWorkComplete: true,
      finalBodyText: finalBodyTxt,
      fixedConditions: firstPresentText(packet.fixedConditions, packet["固定条件"], context.fixedConditions, context.preservedCore)
    });
    failures.push(...titleResult.failures);
    if (A_TO_C_TXTDL_POLICY.selectedTitleMustBeInTitleBlock === true && hasText(titleResult.selectedTitle)) {
      const lines = nonEmptyLines(finalBodyTxt);
      const workTitle = firstPresentText(packet.workTitle, packet["作品タイトル"], context.workTitle, context["作品タイトル"]);
      const partLabel = firstPresentText(packet.partLabel, packet["部表記"], packet["編表記"], packet["部・編等"], context.partLabel, context["部表記"], context["編表記"], context["部・編等"]);
      const episodeLabel = firstPresentText(packet.episodeLabel, packet.storyNumberLabel, packet["話数"], packet["話番号"], packet.targetRange, titleReport["対象範囲"], context.episodeLabel, context.storyNumberLabel, context["話数"], context["話番号"], context.targetRange);
      const expectedTitleBlock = [];
      if (hasText(workTitle)) expectedTitleBlock.push(workTitle);
      if (hasText(partLabel)) expectedTitleBlock.push(partLabel);
      if (hasText(episodeLabel)) expectedTitleBlock.push(episodeLabel);
      else failures.push(fail("EPISODE_LABEL_REQUIRED_FOR_BODY_TITLE_BLOCK", "話数|targetRange"));
      expectedTitleBlock.push(titleResult.selectedTitle);
      for (const [index, expectedLine] of expectedTitleBlock.entries()) {
        if (lines[index] !== expectedLine) {
          failures.push(fail("TITLE_BLOCK_LINE_MISMATCH", `TS90_FINAL_BODY.txt[${index}]`));
        }
      }
    }
  }

  if (isObject(titleReport) && hasText(workReportTxt)) {
    failures.push(...workReportBindingFailures(workReportTxt, titleReport, packet, context));
  }

  return Object.freeze({
    decision: failures.length === 0 ? "A_TO_C_TXTDL_PASS" : "A_TO_C_TXTDL_STOP",
    files: Object.freeze(fileKeys.filter((key) => A_TO_C_TXTDL_SUCCESS_ALLOWED_FILES.includes(key))),
    failures: Object.freeze(failures)
  });
}
...
export function evaluateToshiOutput({ receiveResult, output, textOnlyRequested = false }) {
  const boundary = jsonBoundaryStop([{ receiveResult, output, textOnlyRequested }], "OUTPUT_STOP", "output");
  if (boundary) return boundary;
