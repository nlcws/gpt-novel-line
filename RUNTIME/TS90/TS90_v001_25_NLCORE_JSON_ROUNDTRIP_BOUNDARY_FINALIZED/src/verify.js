...
const titleDecisionChecks = TITLE_DECISION_LOCK_MANIFEST.map(check);
const aToCTxtdlChecks = A_TO_C_TXTDL_LOCK_MANIFEST.map(check);
const skillRuntimeEvidenceChecks = SKILL_RUNTIME_EVIDENCE_LOCK_MANIFEST.map(check);
...
const titleDecisionIdentical = titleDecisionChecks.every((item) => item.identical);
const aToCTxtdlIdentical = aToCTxtdlChecks.every((item) => item.identical);
const skillRuntimeEvidenceIdentical = skillRuntimeEvidenceChecks.every((item) => item.identical);
const decision = sourceIdentical && locksIdentical && textReceiveIdentical && phaseSelfDirectedIdentical && historyMasterReapplyIdentical && fullRevisionIdentical && adaptiveEditorIdentical && titleDecisionIdentical && aToCTxtdlIdentical && skillRuntimeEvidenceIdentical
...
console.log(JSON.stringify({
  ...
  titleDecisionIdentical,
  aToCTxtdlIdentical,
  skillRuntimeEvidenceIdentical,
  sourceChecks,
  lockChecks,
  textReceiveChecks,
