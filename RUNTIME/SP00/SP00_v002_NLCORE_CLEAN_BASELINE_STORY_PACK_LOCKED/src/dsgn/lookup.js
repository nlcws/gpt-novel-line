...
export function validateLookup(input) {
  ...
  if ((lookup.oldAliases ?? []).some((value) => value === "変奏あり")) {
    issues.push(issue("OLD_ALIAS_SINGLE_VALUE", "runtime.lookup.oldAliases",
      "変奏ありを単一値として使えません。sentence_variationの分解lookupが必要です"));
  }
  for (const request of lookup.requests ?? []) {
    if (!request?.tag?.startsWith("sp00.")) {
      issues.push(issue("LOOKUP_TAG_INVALID", "runtime.lookup.requests", "lookupにはsp00.* tagが必要です"));
    }
    if (!request?.sourceId?.startsWith("SP00.SRC.")) {
      issues.push(issue("LOOKUP_SOURCE_UNRESOLVED", "runtime.lookup.requests",
